...
19:15 a 20:15`,trainer:"Nacho"},{days:"Martes y Jueves",time:"19:00 a 20:00",trainer:"Nacho"}]}],r=n.find(s=>s.name===t)||n[0];return f.jsx("section",{className:"schedule-section",id:"schedule",children:f.jsxs("div",{className:"container",children:[f.jsxs("div",{className:"section-header text-center",children:[f.jsxs("h2",{children:["HORARIOS DE ",f.jsx("span",{className:"text-gold",children:"BATALLA"})]}),f.jsx("p",{className:"subtitle",children:"ELIGE TU DISCIPLINA"})]}),f.jsx("div",{className:"filter-container",children:n.map(s=>f.jsx("button",{className:`filter-btn ${t===s.name?"active":""}`,onClick:()=>e(s.name),children:s.name.toUpperCase()},s.name))}),f.jsx("div",{className:"schedule-cards",children:r.schedules.map((s,i)=>{const o=s.trainer.includes("&")||s.trainer.includes(",");return f.jsxs("div",{className:"class-card animate-fade-in",children:[f.jsx("h4",{className:"font-bold mb-2",children:s.days}),f.jsx("p",{className:"text-gold text-2xl font-heading mb-1",style:{whiteSpace:"pre-line"},children:s.time}),f.jsxs("p",{className:"mb-4 text-muted",children:[f.jsx("span",{className:"text-gold font-bold",children:o?"JARLS":"JARL"})," a cargo: ",f.jsx("span",{className:"text-light",children:s.trainer})]}),f.jsx("button",{className:"book-btn w-100",disabled:!0,style:{opacity:.6,cursor:"not-allowed",border:"1px dashed var(--border-color)",color:"var(--text-muted)"},children:"RESERVAS PROXIMAMENTE"})]},i)})})]})})},QD="542317533963",YD=({plan:t,isOpen:e,onClose:n})=>{const{currentUser:r}=ps(),[s,i]=b.useState({nombre:"",email:"",dni:"",telefono:"",direccion:"",genero:""}),[o,l]=b.useState(!1);if(b.useEffect(()=>{r&&e&&i(p=>({...p,nombre:r.displayName||"",email:r.email||""}))},[r,e]),!e||!t)return null;const u=p=>{i({...s,[p.target.name]:p.target.value})},c=async p=>{p.preventDefault(),l(!0);try{const m=ip(H0(hp,"inscriptions")),g=m.id,S={userId:(r==null?void 0:r.uid)||null,nombre:s.nombre,email:s.email,dni:s.dni,telefono:s.telefono,direccion:s.direccion,genero:s.genero,plan:{name:t.name,price:t.price,period:t.period||""},createdAt:J2()},k=tD(m,S),P=new Promise(A=>setTimeout(A,2e3));try{await Promise.race([k,P])}catch(A){console.warn("Ignorando error o timeout de DB para no bloquear al usuario:",A)}const D=`${window.location.origin}/inscripcion/${g}`,w=`⚔️ ¡Skål Valhalla! Soy *${s.nombre}* (DNI: ${s.dni}).
